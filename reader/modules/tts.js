...
export function updateTTSIcon() {
  ...
  if (els.ttsToggle) {
    // The button may hold an emoji rather than an <svg>; create one on demand.
    let svg = els.ttsToggle.querySelector("svg");
    if (!svg) {
      els.ttsToggle.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"></svg>';
      svg = els.ttsToggle.querySelector("svg");
    }
    if (isPlaying) {
