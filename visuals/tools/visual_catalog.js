...
function canonicalAssetPath(value) {
  ...
  if (canonical === '.' || canonical.startsWith('../') || canonical.includes('/../')) {
    throw new Error('Asset path cannot leave the visuals directory');
  }
  if (!/^characters\/[a-z0-9_]+\/images\//.test(canonical)) {
    throw new Error('Character reference images must live under characters/<character_id>/images/');
  }
  return canonical;
}
...
function readCharacterLibrary(directory) {
  const characters = { records: [], issues: [] };
  const referenceSets = { records: [], issues: [] };
  if (!fs.existsSync(directory)) return { characters, referenceSets };

  for (const entry of fs.readdirSync(directory, { withFileTypes: true })
    .filter((item) => item.isDirectory())
    .sort((a, b) => a.name.localeCompare(b.name))) {
    const characterDir = path.join(directory, entry.name);
    const profilePath = path.join(characterDir, 'profile.json');
    if (!fs.existsSync(profilePath)) {
      characters.issues.push(issue(profilePath, '', 'missing_profile', `Missing profile.json for ${entry.name}`));
      continue;
    }

    try {
      const record = JSON.parse(fs.readFileSync(profilePath, 'utf8'));
      characters.issues.push(...validateCharacterRecord(record, profilePath));
      characters.records.push({ record, filePath: profilePath, directoryName: entry.name });
    } catch (error) {
      characters.issues.push(issue(profilePath, '', 'invalid_json', error.message));
    }

    const versionsDir = path.join(characterDir, 'versions');
    const loadedVersions = readJsonDirectory(versionsDir, validateReferenceSetRecord);
    referenceSets.issues.push(...loadedVersions.issues);
    referenceSets.records.push(...loadedVersions.records.map((loaded) => ({
      ...loaded,
      directoryName: entry.name,
    })));
  }

  return { characters, referenceSets };
}
...
class VisualCatalog {
  ...
  static load(baseDir = path.join(__dirname, '..'), { validateAssets = true } = {}) {
    ...
    const library = readCharacterLibrary(path.join(root, 'characters'));
    const characterResult = library.characters;
    const referenceSetResult = library.referenceSets;
    ...
    const assetPathOwners = new Map();
    const assetHashOwners = new Map();

    for (const { record, filePath, directoryName } of characterResult.records) {
      if (directoryName !== record.character_id) {
        issues.push(issue(filePath, 'character_id', 'directory_mismatch', 'Character directory must match character_id'));
      }
    ...
    for (const { record, filePath, filename, directoryName } of referenceSetResult.records) {
      ...
      if (directoryName !== record.character_id) {
        issues.push(issue(filePath, 'character_id', 'directory_mismatch', 'Reference Set must live in its character directory'));
      }
      ...
      if (Array.isArray(record.references)) {
        record.references.forEach((reference, index) => {
          ...
          const pathOwner = typeof reference.path === 'string' ? assetPathOwners.get(reference.path) : null;
          if (pathOwner && pathOwner !== record.character_id) {
            issues.push(issue(
              filePath,
              `references[${index}].path`,
              'cross_character_asset_path',
              `Asset path ${reference.path} is already owned by Character Identity ${pathOwner}`,
            ));
          } else if (typeof reference.path === 'string') {
            assetPathOwners.set(reference.path, record.character_id);
          }

          const hashOwner = typeof reference.sha256 === 'string' ? assetHashOwners.get(reference.sha256) : null;
          if (hashOwner && hashOwner !== record.character_id) {
            issues.push(issue(
              filePath,
              `references[${index}].sha256`,
              'cross_character_asset_hash',
              `Asset hash ${reference.sha256} is already owned by Character Identity ${hashOwner}`,
            ));
          } else if (typeof reference.sha256 === 'string') {
            assetHashOwners.set(reference.sha256, record.character_id);
          }
        });
      }
    ...
    if (issues.length > 0) {
      throw new CatalogValidationError(`Visual Catalog contains ${issues.length} validation issue(s)`, issues);
    }
    ...
    return new VisualCatalog(root, characters, referenceSets);
  }
  ...
  getCharacter(characterId, { required = true } = {}) {
    ...
    if (!character) {
      ...
      throw new CatalogValidationError(`Unknown character: ${characterId}`, [
        issue('VisualCatalog', 'character_id', 'unknown_character', `Unknown character: ${characterId}`),
      ]);
    }
  ...
  getReferenceSet(referenceSetId, { required = true } = {}) {
    ...
    if (!referenceSet) {
      ...
      throw new CatalogValidationError(`Unknown Reference Set: ${referenceSetId}`, [
        issue('VisualCatalog', 'reference_set_id', 'unknown_reference_set', `Unknown Reference Set: ${referenceSetId}`),
      ]);
    }
  ...
  resolveCharacterId(nameOrId, { required = true } = {}) {
    ...
    if (!characterId && required) {
      throw new CatalogValidationError(`Unknown character name: ${nameOrId}`, [
        issue('VisualCatalog', 'name', 'unknown_character_name', `Unknown character name: ${nameOrId}`),
      ]);
    }
    return characterId;
  }
  ...
  _enrichCharacter(character) {
    ...
    // better in the Reader, so surface a separate display-first pick.
